...
async function md5(message: string): Promise<string> {
  ...
  // Web Crypto doesn't support MD5, so we use a pure JS implementation
  return md5Hash(data)
}

// Pure JS MD5 implementation
function md5Hash(bytes: Uint8Array): string {
  ...
  const padded = new Uint8Array(((n + 72) & ~63) + 64)
  padded.set(bytes)
  padded[n] = 0x80
  const bits = n * 8
